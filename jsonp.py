...
def AssertRateLimitNotExceeded(client_ip):
  ...
  cache_key = IP_CACHE_KEY_PREFIX + client_ip
...
def FetchJson(url, client_ip, use_cache):
  ...
  """
  url = SanitizeUrl(url)
  cache_key = URL_CACHE_KEY_PREFIX + url
  value = None
  if use_cache:
...
class Jsonp(webapp2.RequestHandler):
  ...
  def get(self):
    ...
    try:
      json_data = FetchJson(url, self.request.remote_addr, use_cache)
      if hl:
