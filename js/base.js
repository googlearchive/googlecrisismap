...
/**
 * @desc Label for a checkbox to use the current map viewport to set the
 *     layer viewport, as an alternative to typing in numbers for the viewport.
 */
...
/** @desc Label for a checkbox that controls whether to show a download link. */
...
/** @desc Label for a text field for entering the ID of a Fusion Table. */
cm.MSG_GFT_TABLE_ID = goog.getMsg('Table ID');

/** @desc Label for a text field for entering the name of a table column. */
 ...
 * ready; at that time, wrap this message in a call to goog.getMsg();
 *
 * desc Label for a text field for entering a condition that will be used
 *     to filter the data to be shown on the map.  An example of a condition
 *     would be "type = 'tornado'" or "speed > 60".
 */
...
/**
 * @desc Label for a menu for selecting the speed unit to use for displaying
...
/** @desc Label for a list of layers that the user can select. */
...
/**
 * @desc Tooltip for a text field for entering the ID of a Fusion Table
 *     ("Google Fusion Tables" is the name of a Google product).
 */
...
/**
 * @desc Tooltip for a text field for entering the name of a table column.
 *     "Fusion Table" means a table in the Google Fusion Tables product.
 *     Note that the link URL points to a Help Center page, and its "hl=en"
 *     parameter should be adjusted to point to the appropriate language.
 */
cm.MSG_GFT_LOCATION_COLUMN_TOOLTIP = goog.getMsg(
    'The name of the <a target="_blank" href="http://support.google.com/' +
    'fusiontables/answer/2590990?hl=en&ref_topic=2573808">location column</a>' +
...
cm.UNTRANSLATED_MSG_GFT_FILTER_CONDITION_TOOLTIP =
    'A condition used to filter the data to be drawn on the map, such as ' +
    '<b><code>type = \'tornado\'</code> AND speed &gt; 60</code></b>. ' +
    ...
    'If this is left blank, all the data in the table is drawn on the map.';
...
/** @desc Tooltip for the selection list of layers for a WMS service. */
