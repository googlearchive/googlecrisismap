  ...
  def Get(self):
    ...
    def SortKey(user):
      ...
      if user.id.startswith('test'):
        return (1, user.domain, int(user.id[4:]))
      return (2, user.domain, user.email)
    previous_users = sorted(users.GetAll(), key=SortKey)

    new_users = []
    if not any(u.id == 'root' for u in previous_users):
      new_users = [users.User(id='root', domain=None, email='root@app.com')]
    old_uids = [u.id for u in previous_users if u.id.startswith('test')]
    uid = 'test%d' % (max(int(uid[4:]) for uid in ['test0'] + old_uids) + 1)
    new_users += [
        users.User(id=uid, domain=None, email=uid + '@gmail.test'),
        users.User(id=uid, domain='alpha.test', email=uid + '@alpha.test'),
        users.User(id=uid, domain='beta.test', email=uid + '@beta.test')
    ]
